...
function startClock() {
  ...
  if (changingColor) {
    const mainColor = dateToHexColor(date);
    const secondaryColor = generateColor(mainColor, 8, 15, 15);
    const thirdColor = generateColor(mainColor, 2, 5, 5);
...
function dateToHexColor(date) {
  ...
  let dateForHex =
    '#' +
    format(hours % 12).toString() +
    format((Math.abs(minutes - 20) / 2).toString()) +
    format((seconds / 2).toString());
...
const toggleDynamicColorsButton = document.getElementById(
  'toggleDynamicColors'
);

toggleDynamicColorsButton.addEventListener('click', () => {
